...
echo "[INIT] Starting reference UI on port 8085..."
npx http-server /workspace/ui -p 8085 -a 0.0.0.0 >/dev/null 2>&1 &
UI_PID=$!

echo "[WAIT] Checking http://localhost:8085 ..."
for i in {1..30}; do
...
find tests -type f -name "*.ts" | while read -r file; do
  if grep -q "@playwright/test" "$file"; then
    # Replace import from @playwright/test with coverage setup
